...
<mx:contentBean cid="${cid}"/><!-- 根据id查询新闻内容 -->
<mx:contentMap cid="${cid}"/>
<!DOCTYPE HTML>
<html>
  <head>
    <title>彩票圈-${content.title}</title>
 ...
 <body>
	...
	<div class="box">
		<div class="location">
			<div class="left">
				您现在的位置：<a href="/" target="_self">彩票圈</a> >> <a href="/${contentMap.chDesc}" target="_self">${contentMap.pname}</a> >> <a href="/web/content/more/${contentMap.chid}" target="_self">${contentMap.cname}</a> >> 正文
			</div>
		</div>
	</div>
		...
		<div class="detail">
			<div class="left">
				<div class="content" id="leftcnt">
					...
					<h2>${content.title}</h2>
					<div class="fix"></div>
					<div class="about">标签：${content.tag}&nbsp;&nbsp;&nbsp;<span>时间：${mx:formatDate(content.createTime,'yyyy-MM-dd')}</span></div>
					<div class="fix"></div>
					<div class="fix"></div>
					<!-- 内容详情 -->
					<div class="text">
